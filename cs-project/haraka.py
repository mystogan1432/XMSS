...
Python implementation of Haraka v2, an efficient short-input hash function
...
RC = [
    int.to_bytes(c, 16, 'little') for c in [
        0x0684704ce620c00ab2c5fef075817b9d, 0x8b66b4e188f3a06b640f6ba42f08f717,
...
def aesenc(state: bytes, round_key: bytes) -> bytes:
    """
    Compute single round of AES
    """
    enc = [
        AES_TE0[state[0]] ^ AES_TE1[state[5]] ^ AES_TE2[state[10]] ^ AES_TE3[state[15]],
        AES_TE0[state[4]] ^ AES_TE1[state[9]] ^ AES_TE2[state[14]] ^ AES_TE3[state[3]],
        AES_TE0[state[8]] ^ AES_TE1[state[13]] ^ AES_TE2[state[2]] ^ AES_TE3[state[7]],
        AES_TE0[state[12]] ^ AES_TE1[state[1]] ^ AES_TE2[state[6]] ^ AES_TE3[state[11]]
    ]
    state = b"".join(int.to_bytes(c, 4, 'big') for c in enc)
    return xor(state, round_key)


def mix512(state: List[bytes]) -> List[bytes]:
    """
    Linear mixing for Haraka-512/256
    """
    return [state[0][12:16] + state[2][12:16] + state[1][12:16] + state[3][12:16],
            state[2][0:4] + state[0][0:4] + state[3][0:4] + state[1][0:4],
            state[2][4:8] + state[0][4:8] + state[3][4:8] + state[1][4:8],
            state[0][8:12] + state[2][8:12] + state[1][8:12] + state[3][8:12]]


def mix256(state: List[bytes]) -> List[bytes]:
    """
    Linear mixing for Haraka-256/256
    """
    return [state[0][0:4] + state[1][0:4] + state[0][4:8] + state[1][4:8],
            state[0][8:12] + state[1][8:12] + state[0][12:16] + state[1][12:16]]
...
def haraka256(msg: bytes) -> bytes:
    ...
    state = [msg[i:i+16] for i in range(0, 32, 16)]

    # apply round functions
    for t_round in range(ROUNDS):
        for m_round in range(AES_PER_ROUNDS):
            state = [aesenc(state[i], RC[2 * t_round * AES_PER_ROUNDS + 2 * m_round + i])
                     for i in range(2)]
    ...
    # apply feed-forward
    state = [xor(state[i], msg[16*i:16*i+16]) for i in range(2)]
...
def haraka512(msg: bytes) -> bytes:
    ...
    """
    # obtain state from msg input
    state = [msg[i:i+16] for i in range(0, 64, 16)]

    # apply round functions
    for t_round in range(ROUNDS):
        for m_round in range(AES_PER_ROUNDS):
            state = [aesenc(state[i], RC[4 * t_round * AES_PER_ROUNDS + 4 * m_round + i])
                     for i in range(4)]
    ...
    # apply feed-forward
    state = [xor(state[i], msg[16 * i:16 * i + 16]) for i in range(4)]

    # truncation
    return state[0][8:] + state[1][8:] + state[2][0:8] + state[3][0:8]


def check_test_vectors():
    """
    Check test vectors. Values are taken from the original Haraka paper.
    """
    test_value = b'\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f' \
                 b'\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1a\x1b\x1c\x1d\x1e\x1f' \
                 b'\x20\x21\x22\x23\x24\x25\x26\x27\x28\x29\x2a\x2b\x2c\x2d\x2e\x2f' \
                 b'\x30\x31\x32\x33\x34\x35\x36\x37\x38\x39\x3a\x3b\x3c\x3d\x3e\x3f'
    hash512 = b'\xbe\x7f\x72\x3b\x4e\x80\xa9\x98\x13\xb2\x92\x28\x7f\x30\x6f\x62' \
              b'\x5a\x6d\x57\x33\x1c\xae\x5f\x34\xdd\x92\x77\xb0\x94\x5b\xe2\xaa'
    hash256 = b'\x80\x27\xcc\xb8\x79\x49\x77\x4b\x78\xd0\x54\x5f\xb7\x2b\xf7\x0c' \
              b'\x69\x5c\x2a\x09\x23\xcb\xd4\x7b\xba\x11\x59\xef\xbf\x2b\x2c\x1c'

    assert haraka512(test_value) == hash512
    assert haraka256(test_value[:32]) == hash256
